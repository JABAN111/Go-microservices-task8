...
type ClientManager struct {
	clients map[string]ports.GrpcClient
	mu      sync.RWMutex
...
func NewClientManager(log *slog.Logger) *ClientManager {
	...
	return &ClientManager{
		clients: make(map[string]ports.GrpcClient),
		log:     log,
	}
}

func (cm *ClientManager) Register(name string, client ports.GrpcClient) {
	cm.mu.Lock()
	defer cm.mu.Unlock()
...
func (cm *ClientManager) GetClient(name string) (ports.GrpcClient, error) {
	cm.mu.RLock()
	defer cm.mu.RUnlock()
...
func (cm *ClientManager) CloseAll(ctx context.Context) {
	...
	for _, client := range cm.clients {
		wg.Add(1)
		go func(client ports.GrpcClient) {
			defer wg.Done()
