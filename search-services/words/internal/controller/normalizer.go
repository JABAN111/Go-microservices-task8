...
// TODO: где-то здесь заплакал Роберт Мартин и dry.
// FIXME объединить GetWords c Norm
func (n *normalize) GetWords(ctx context.Context, in *wordspb.WordsRequest) (*wordspb.WordsReply, error) {
	reqCtx, reqCancel := context.WithCancel(ctx)
	defer reqCancel()
	...
	words, err := normalizeService.GetWords(reqCtx, in.Phrase)
	if err != nil {
		log.Warn("Error has occured while getting unique words", "err", err)
		return nil, status.Error(codes.Internal, err.Error())
	}
